...
import AddressPopup from './components/Address/AddressPopup.jsx';
...
function Checkout() { 
    ...
    const refreshCheckoutData = async () => { 
        console.log('refreshCheckoutData');
        ...
        myaxios.post('/checkout/check', {
            ...
            .then(API => {
                console.log('refreshCheckoutData finish');
    ...
    return (
        <div className={cn('container')}>
                    ...
                    <div className={cn('checkout_voucher')}>
                        <div className={cn('checkout_voucher__bar')}>
                            ...
                            <div className={cn('bar-right')}>
                                <input type="text" placeholder={voucherInput===''?((checkoutData.voucher.code!=='' && checkoutData.voucher.is_valid)?checkoutData.voucher.code:"Nhập mã voucher ngay để được giảm giá"):voucherInput}
                                    ref={inputVoucherRef}
                                    value={voucherInput}
                                    onChange={(e) => { 
                                ...
                                <button className={cn('submit-voucher-button', {'active': true})}
                                    onClick={() => {
                                        if (voucherInput !== checkoutData.voucher.code)
                                            refreshCheckoutData()
                                    }}
                                >ÁP DỤNG</button>
                            </div>
            ...
            {
                (showAddressPopup || (checkoutData.ship && checkoutData.ship.to_address_id === -1))
                &&
