...
export default new class CartController {
    ...
    // [POST] /cart/remove-one-product-to-cart
    async removeOneProductFromCart(req, res) {
        let credentials_info = (req.session && req.session.user)?req.session.user:{};
        let product_id = req.query.product_id?req.query.product_id:(-1);

        if (credentials_info.id) { 
            (async () => {
                try {
                    const code_success = await CartModel.removeOneProductFromCart(credentials_info.id, product_id)
    ...
    async removeManyProductsFromCart(req, res) {
        let credentials_info = (req.session && req.session.user)?req.session.user:{};
        let product_id_list = req.body.product_id_list?JSON.parse(req.body.product_id_list):[];
        ...
        if (credentials_info.id) { 
            (async () => {
                try {
                    const code_success = await CartModel.removeManyProductsFromCart(credentials_info.id, product_id_list)
