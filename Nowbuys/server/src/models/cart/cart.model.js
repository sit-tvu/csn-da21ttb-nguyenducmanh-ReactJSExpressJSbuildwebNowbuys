...
export default new class CartModel {
    ...
    async removeOneProductFromCart(user_id, product_id) {
        return await new Promise((resolve, reject) => {
            let sql = `DELETE FROM cart WHERE user_id = ${user_id} AND product_id = ${product_id}`;
            console.log(sql);
    ...
    async removeManyProductsFromCart(user_id, product_id_list) {
        return await new Promise((resolve, reject) => {
